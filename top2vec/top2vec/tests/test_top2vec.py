...
import gensim
# 中文预料
with open("C:\\Users\\hp\\PycharmProjects\\final_project\\stop_list.txt", 'r', encoding='utf-8') as r:
    stop_list = r.readlines()
with open("C:\\Users\\hp\\PycharmProjects\\final_project\\data\\corpus_seg.txt", 'r',encoding='utf-8') as doc:
    docs = doc.readlines()
train_docs = []
for i, text in enumerate(docs):
    # word_list = text.split(' ')
    word_list_tmp = jieba.lcut(text)
    word_list = " ".join(e for e in word_list_tmp if e not in stop_list)
    train_docs.append(word_list)
newsgroups_documents = train_docs


# get 20 newsgroups data
newsgroups_train = fetch_20newsgroups(subset='all', remove=('headers', 'footers', 'quotes'))
newsgroups_train.data = newsgroups_documents
